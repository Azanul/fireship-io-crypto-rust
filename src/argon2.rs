...
pub fn signup(email: String, password: String) -> String{
    ...
    let mut old_users: Vec<User> = serde_json::from_str(&fs::read_to_string("users_argon.json").expect("Unable to read file")).unwrap();
    let new_user = User {email, password};
    old_users.push(new_user);

    fs::write("users_argon.json", serde_json::to_string_pretty(&old_users).unwrap()).expect("Unable to write file");
...
pub fn login(email: String, password: String) -> String {
    ...
    let users: Vec<User> = serde_json::from_str(&fs::read_to_string("users_argon.json").expect("Unable to read file")).unwrap();
